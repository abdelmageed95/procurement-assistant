...
2. **Custom Judges**: Five LLM-as-judge evaluators using `mlflow.genai.make_judge()`
...
- **Response Judges**: Natural Language and Relevance judges evaluate the response
...
**Category 2: Response Quality (20 points)**
...
semantic_judge = mlflow.genai.make_judge(
    ...
    instructions="""Evaluate if the MongoDB query semantically matches the user's intent.
...
Experiment: procurement-assistant-evaluation
    ...
    +-- Run: eval_20251030_214449
        ...
        |
        +-- Traces
            - Query 1 trace (with all 5 judge assessments)
            - Query 2 trace (with all 5 judge assessments)
...
- Syntax Correctness: 33/35
- Semantic Correctness: 28/30
- Query Efficiency: 14/15
- Natural Language: 14/15
- Relevance: 5/5
